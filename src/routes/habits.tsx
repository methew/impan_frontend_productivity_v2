...
function HabitsPage() {
  ...
  const [editHabitColor, setEditHabitColor] = useState(COLOR_OPTIONS[0])
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 overflow-hidden p-4">
        <HabitTracker
          ...
          onEdit={(habit) => {
            ...
            setEditHabitColor(habit.color || COLOR_OPTIONS[0])
            setEditTargetDays(habit.target_days || 7)
      ...
      <Dialog open={showEditDialog} onOpenChange={setShowEditDialog}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t('habits.editHabit', '编辑习惯')}</DialogTitle>
          </DialogHeader>
              ...
              <div className="flex flex-wrap gap-2 mt-2">
                {COLOR_OPTIONS.map(color => (
                  <button
                    ...
                    className={`w-8 h-8 rounded-full transition-all ${
                      editHabitColor === color ? 'ring-2 ring-offset-2 ring-primary' : ''
                    }`}
                    style={{ backgroundColor: color }}
                  />
                ))}
            ...
            <div>
              ...
              <div className="flex items-center gap-4 mt-2">
                <input
                  ...
                  className="flex-1"
                />
                <span className="text-sm font-medium w-8">{editTargetDays}</span>
              </div>
            </div>
          </div>
          ...
          <DialogFooter>
            ...
            <Button 
              onClick={handleEdit} 
              disabled={!editHabitName.trim() || updateHabit.isPending}
            >
              {updateHabit.isPending ? '保存中...' : t('common.save', '保存')}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
