...
import { useInbox, useCreateTask, useCompleteTask, useUpdateTask, filterTasks } from '@/hooks/useTasks'
...
function InboxPage() {
  ...
  const filteredTasks = useMemo(() => {
    return filterTasks<Task & { tags?: Tag[] }>(tasks, searchQuery)
  }, [tasks, searchQuery])
  ...
  const buildTreeItems = (taskList: Task[]): OutlineItem[] => {
    
    // Get root level tasks (no parent)
    const rootTasks = taskList.filter(t => !t.parent)
