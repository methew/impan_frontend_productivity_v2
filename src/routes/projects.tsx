...
export function ProjectsPage() {
  ...
  const [viewFilter, setViewFilter] = useState<'available' | 'remaining' | 'all'>('all')
  ...
  const getProjectTasks = useCallback((projectId: string): Task[] => {
    if (!allTasks) return []
    return allTasks.filter(t => String(t.project) === projectId)
  }, [allTasks])
  ...
  const handleTreeMove = (activeId: string, overId: string | null, position: DropPosition) => {
    ...
    // Update parent relationship via API
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" size="sm" className="h-7 text-xs">
                  {t('projects.view')}: {viewFilter === 'available' ? t('projects.available') : viewFilter === 'remaining' ? t('projects.remaining') : t('common.all')}
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end">
                <DropdownMenuItem 
                ...
                >
                  {viewFilter === 'available' && <Check className="h-3 w-3 mr-2" />}
                ...
                >
                  {viewFilter === 'remaining' && <Check className="h-3 w-3 mr-2" />}
                ...
                >
                  {viewFilter === 'all' && <Check className="h-3 w-3 mr-2" />}
                  {t('common.all')}
                </DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
...
interface TreeOutlineProps {
  ...
  viewFilter?: 'available' | 'remaining' | 'all'
  searchQuery?: string
}
...
function TreeOutline({
  ...
  // Filter tasks based on view filter and search query
  const filteredTasks = useMemo(() => {
    let tasks = allTasks
    
    // Apply view filter
    if (viewFilter === 'available') {
      tasks = tasks.filter(t => !t.completed_at && !t.dropped_at && (!t.defer_date || new Date(t.defer_date) <= new Date()))
    } else if (viewFilter === 'remaining') {
      tasks = tasks.filter(t => !t.completed_at && !t.dropped_at)
    }
    ...
    return tasks
  }, [allTasks, viewFilter, searchQuery])
  ...
  useEffect(() => {
    ...
    if (viewData.type === 'folder') {
      ...
      folderProjects.forEach(project => {
        const ptasks = filteredTasks?.filter(t => String(t.project) === String(project.id) && !t.parent) || []
        if (ptasks.length > 0) {
  ...
  }, [viewData.type, viewData.folder?.id, viewData.project?.id, folders, allProjects, filteredTasks])

  const getProjectTasks = (projectId: string | number): Task[] => {
    return filteredTasks?.filter(t => String(t.project) === String(projectId) && !t.parent) || []
  }

  const getChildTasks = (parentId: string): Task[] => {
    return filteredTasks?.filter(t => String(t.parent) === parentId) || []
  }
    ...
    if (e.altKey || e.metaKey) {
      ...
      if (task.completed_at) {
        ...
        updateTask.mutate({ 
          ...
          onSuccess: () => toast.success(t('projects.status.completed') + ' - ' + t('common.success')),
          onError: (err) => toast.error(t('projects.updateFailed'), { description: err.message })
        })
      } else {
        // Mark as completed
